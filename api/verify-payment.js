...
const PRICE_SOL     = parseFloat(process.env.PRICE_SOL     || '0.5');  // builder campaigns
const FUD_PRICE_SOL = parseFloat(process.env.FUD_PRICE_SOL || '1.0');  // FUD submissions
...
module.exports = async (req, res) => {
  ...
  const { signature, campaignId, type } = req.body || {};
  const isFudSubmission = type === 'fud';
  ...
  const requiredLamports = Math.floor(
    (isFudSubmission ? FUD_PRICE_SOL : PRICE_SOL) * LAMPORTS_PER_SOL
  );
  const requiredUsdcMicro = Math.floor(PRICE_USDC * 1_000_000);
  ...
  if (tIdx !== -1) {
    ...
    if (gained >= requiredLamports) validPayment = true;
  }

  if (!validPayment && !isFudSubmission) {
    ...
    for (const p of post) {
      if (p.mint === USDC_MINT && p.owner === TREASURY) {
        ...
        if (postAmt - preAmt >= BigInt(requiredUsdcMicro)) {
          validPayment = true;
          break;
        }
      }
  ...
  try {
    if (isFudSubmission) {
      // FUD submissions go into review queue — admin must approve
      ...
      // Builder campaigns go live immediately
      await db.updateDocument(DB, COLL, campaignId, {
        active:      true,
        txSignature: signature,
      });
  ...
  return res.status(200).json({ success: true, pending: isFudSubmission });
};
